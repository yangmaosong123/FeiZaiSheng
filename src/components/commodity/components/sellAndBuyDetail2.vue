...
<div class="sell-and-buy-detail main">
    <div class="container">
        <!--标题-->
        <CommonTitle :title="this.$route.query.tradeType=='buy'?'求购详情':this.$route.query.tradeType=='sell'?'出售详情':''">
        </CommonTitle>
        <section>
            <el-row>
                <el-col :span="5" class="enterprise">
                    <!-- <h3>搜索栏目</h3> -->
                    <div>
                        <img  :src="$store.state.outputImgByIdUrl+detail.unitLogoAttId"
                                :alt="detail.unitLogoAttId+'img'">
                       
                            <h4>{{detail.enterpriseName}}</h4>
                            <p>企业信誉：优</p>
                            <div class="collection">
                                <el-button type="primary" v-show="showRemarkCompanyBtn" @click="handleCollectionEnterprise">收藏企业</el-button>
                                <el-button type="primary" v-show="showRemarkGoodsBtn" @click="handleProductEnterprise(detail.id)">收藏商品</el-button>
                            </div>
                ...
                <el-col :span="18" class="commodity">
                    <el-col :span="11">
                        <div>
                            <BabyByCarousel :smallImages="smallImages" :contentBoxDiv="contentBoxDiv" :bigImgStyle="bigImgStyle" :smallImgStyle="smallImgStyle">
                            </BabyByCarousel>
                            <!--宝贝轮播图 -->
                        </div>
                    </el-col>
                    <el-col :span="12">
                        <div class="introduce">
                            ...
                            <p>交易种类:{{findConfigItem(detail.source,$store.state.dictionary.hsly).title}}
                            ...
                            <p>
                                数&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;量：{{detail.quantity}}{{findConfigItem(detail.unit,$store.state.dictionary.unit).title}}
                                <span>&nbsp;&nbsp;({{calTradeQuantity}}{{findConfigItem(detail.unit,$store.state.dictionary.unit).title}})</span>
                            </p>
                            <p>
                                物&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;流：{{detail.logisticsIncluded=='Y'?'卖家送货上门':detail.logisticsIncluded=='N'?'买家上门取货':''}}
                            </p>
                            <div class="describe">
                                <p class="price">
                                    价格：{{!detail.price||detail.price=='0'?'价格面议':'￥'+detail.price+'/'+findConfigItem(detail.unit,$store.state.dictionary.unit).title}}
                                </p>
                            </div>
                            <el-button v-show="showSellBtn" @click="handleShowMySell(detail.minTradeQuantity)">我要出售
                            </el-button>
                            <el-button v-show="showBuyBtn" @click="handleShowMyBuy(detail.minTradeQuantity)">我要购买
                            </el-button>
                        </div>
                    </el-col>
                </el-col>
            ...
            <el-row>
                <el-col :span="5" class="information">
                    <div class="about">
                        <h3>相关资讯</h3>
                        <ul>
                            <li v-for="item in lastUpdates" @click="handleGoToNotice(item.id)" :key="item.id">
                                <a href="#">{{'•'+item.title}}</a>
                            </li>
                        </ul>
                    </div>
                    <div class="demand">
                        <h3>该企业发布过的需求</h3>
                        <div v-for="item in demands" @click="gotoGoodsDetailPage(item)" :key="item.id">
                            <img :src="$store.state.outputImgByIdUrl+item.productFile" :alt="item.productFile+'img'">
                    ...
                    <div class="transaction">
                        ...
                        <el-row>
                            ...
                            <el-col :span="12">
                                ...
                                <p>物流承运：{{detail.logisticsIncluded=='Y'?'买家送货上门':detail.logisticsIncluded=='N'?'买家上门取货':''}}</p>
                                <p>最小成交量：{{detail.minTradeQuantity}}{{findConfigItem(detail.unit,$store.state.dictionary.unit).title}}</p>
                            </el-col>
                        </el-row>
                    </div>
    ...
    <!--我要购买dialog-->
    <el-dialog title="我要购买" :visible.sync="buyDialogVisible" width="30%" @close='closeBuyWin'>
        <el-form :model="buyForm" :rules="buyFormRules" ref="buyForm" class="demo-ruleForm" label-width="100px">
            <el-form-item label="数量" prop="tradeQuantity">
                <el-input-number v-model="buyForm.tradeQuantity" :min="minTradeQuantity" :max="buyForm.maxTradeQuantity"></el-input-number>
            </el-form-item>
        </el-form>
    ...
    <!-- 我要出售 -->
    <el-dialog title="我要出售" :visible.sync="sellDialogVisible" width="30%" @close='closeSellWin'>
        <el-form class="demo-ruleForm" :rules="sellFormRules" ref="sellForm" :model="sellForm" label-width="100px">
            <el-form-item label="数量" prop="tradeQuantity">
                <el-input-number v-model="sellForm.tradeQuantity" :min="minTradeQuantity" :max="buyForm.maxTradeQuantity">
                </el-input-number>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
export default {
    data() {
        ...
        return {
            ...
            buyDialogVisible: false, //购买dialog（隐藏）
            sellDialogVisible: false, //出售dialog(隐藏)
            //购买表单数据
            buyForm: {
                tradeQuantity: 0, //数量
                maxTradeQuantity: 0, //最大成交量
            },
            minTradeQuantity: 0, //最小成交量
            //购买表单验证规则
            buyFormRules: {
                tradeQuantity: [{
                    validator: checkBuy,
            ...
            buyBtn: false, //防止用户多次点击
            //出售表单数据
            sellForm: {
                tradeQuantity: 1 //数量
            },
            sellBtn: false, //防止用户多次点击
            //出售表单验证规则
            sellFormRules: {
                tradeQuantity: [{
                    validator: checkSell,
    ...
    created: function () {
        //alert("dddd")
        this.handleGetDetailData() //获取详情数据
        this.getInformation() //显示资讯数据
        this.handleGetHavePublishedData(); //获取该企业发布过的需求
    },
    computed: {
        //是否显示收藏企业按钮
        ...
        calTradeQuantity: function () {
            var sz = [];
            sz.push(this.detail.tradeType == "buy" ? "成交" : "剩余");
            var tradeQuantity = this.detail.tradeQuantity;
            if (tradeQuantity == null) {
                tradeQuantity = 0;
            }
            sz.push(this.detail.tradeType == "buy" ? tradeQuantity : this.detail.quantity - tradeQuantity);
            return sz.join("");
        },
        //是否显示(我要出售)按钮
        showSellBtn: function () {
            if (this.$store.state.accountObj.companyId == 0) {
                ...
                return (this.$store.state.accountObj.companyId != 0 &&
    ...
    methods: {
        //获取详情数据
        handleGetDetailData: function () {
            let _this = this;
            _this.id = _this.$route.query.id;
            _this.$axios({
                method: "post",
                url: _this.$store.state.recycleAddr + "/recProduct/buyCommodityDetails",
                data: {
                ...
                if (res.data.status == 200) {
                    ...
                    //计算剩余数量
                    _this.buyForm.maxTradeQuantity = Math.max(0, _this.detail.quantity - tradeQuantity);
                    _this.minTradeQuantity = Math.min(_this.buyForm.maxTradeQuantity, _this.detail.minTradeQuantity);
                }
            }).then(function () {
        ...
        //收藏企业按钮事件
        handleCollectionEnterprise: function () {
            //判断是否有登陆
            if (this.checkLogin() == false) {
                return
            }
            let _this = this
            _this.this.$confirm('确定收藏该企业吗', '提示', {
        ...
        //收藏商品事件
        handleProductEnterprise: function (id) {
            let _this = this;
        ...
        //进入企业
        handleGoToCompaniesToRecommend: function (detail) {
            let _this = this
            _this.$router.push({
                path: "/clickToEnter",
                query: {
        ...
        //获取该企业发布过的需求
        handleGetHavePublishedData: function () {
            let _this = this
            _this.$axios({
                method: "post",
                ...
                if (res.data.status == 200) {
                    _this.demands = res.data.data;
                }
            })
        },
        //确定购买
        handleSubmitBuy: function (buyForm) {
            let _this = this;
        ...
        //显示购买弹框dialog
        handleShowMyBuy(minTradeQuantity) {
            if (this.checkLogin() == false) {
                return;
            }
            ...
            this.checkAddr(function (res) {
                if (res.data.status == 200 && res.data.data.list.length > 0) {
                    _this.buyForm.tradeQuantity = minTradeQuantity;
                    _this.buyForm.minTradeQuantity = minTradeQuantity;
        ...
        //显示出售弹框
        handleShowMySell(minTradeQuantity) {
            if (this.checkLogin() == false) {
                return;
            }
            if (this.checkValidQuantity() == false) {
            ...
            this.checkAddr(function (res) {
                if (res.data.status == 200 && res.data.data.list.length > 0) {
                    _this.sellForm.tradeQuantity = minTradeQuantity;
                    _this.sellForm.minTradeQuantity = minTradeQuantity;
        ...
        //关闭出售弹框
        closeSellWin: function () {
            var _this = this;
            _this.$refs["sellForm"].clearValidate();
        },
        //确定出售
        handleSubmitSell: function (sellForm) {
            let _this = this
