...
<div class="baby-by-carousel">
    <slot>
        <div :style="contentBoxDiv" class="baby-by-content"> <!--大图片-->
            <img  class="big-img" :src="$store.state.outputImgByIdUrl+bigImage"  :alt="bigImage+'bigImg'">
            <div class="baby-by-carousel-content">             <!--左箭头符号<-->
...
export default {
    ...
    methods: {
        handleSetBigImageShow: function (item, index) {
            this.itemIndex = index
            this.bigImage = this.smallImages[this.itemIndex]
        },
        //左箭头事件
        handleClickBtnPrev: function () {
            this.itemIndex = Math.max(0, this.itemIndex - 1)
            this.bigImage = this.smallImages[this.itemIndex]
        },
        //右箭头事件
        handleClickBtnNext: function () {
