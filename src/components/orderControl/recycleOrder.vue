<template>
  <div class="recycle-order">
    ...
    <el-table @filter-change="handleFilterChg" ref="tableData" default-expand-all :data="tableData"
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-row id="content">
            <el-col :span="3">
              <div class="logo">
                <img @click="" :src="$store.state.outputImgUrl+props.row.productId+'&bizObjName=upload_img'"
            ...
            <el-col :span="6">
              <el-row class="title">
                <el-col><span @click="">【{{props.row.tradeType=='buy'?'求购':props.row.tradeType=='sell'?'出售':''}}】
                              <el-tooltip placement="right-start" effect="light">
                                <div slot="content">
                                   {{props.row.productTitle}}
                                </div>
                                <el-button class="nowrap4 nowrap-h3"
                                           @click="handleGoToLogisticsDetail(props.row)"
                                           type="text"
                                >
                                   {{props.row.productTitle}}
              ...
              <el-row class="address">
                ...
                <el-row>
                                <span style="white-space:normal; word-break:break-all;">地址：
                                  {{ makeAddr(props.row.province,props.row.city,props.row.region)}}</span>
                </el-row>
              </el-row>
            </el-col>
            ...
            <el-col :span="3" class="details"><!--显示商品发布方在当前交易订单上的状态-->
            ...
            <el-col :span="3" class="operation">
              <el-button type="text" size="small" @click="handleShowPayment(props.row)"
                         v-show="props.row.orderStatusForBuyer=='wait_pay'">付款
              </el-button>
              <el-button type="text" size="small" @click="handleShowOrder(props.row)"
                         v-show="props.row.orderStatusForBuyer=='wait_confirm'">确定订单
              </el-button>
              <el-button type="text" size="small" @click="showConfirmDialog(props.row)"
                         v-show="props.row.orderStatusForBuyer=='convey'&&props.row.tradeType=='sell'">交易确认
              </el-button>
      ...
      <el-table-column prop="tag" label="日期" width="220" column-key="filter_date" :filter-multiple="false"
      ...
      <el-table-column label="订单状态" width="130" column-key="filter_tag" :filter-multiple="false"
                       :filters="dictionaryToFilter($store.state.dictionary.rec_order_status_for_buyer)" prop="tag">
      </el-table-column>
    ...
    <confirm-order @cancel='cancelConfirmOrder' v-if="showConfirmOrder" @after-save="confirmOk"
                   :id="contextRowData.id"/>
...
<script>
  ...
  import ConfirmOrder from '@/components/rec/ConfirmOrder'
  ...
  export default {
    ...
    methods: {
      ...
      handleConfirmOrder: function (flag) {
        this.showConfirmOrder = false;
        if (flag == "Y" && this.contextRowData.tradeType == "sell") {
          this.showConfirmDialog(this.contextRowData);
        } else {
          this.confirmOk();
        }
